...
def Agentshare(currAgent, iR,Asend,bsend,Asendbuff,bsendbuff,Areceivelocal,breceivelocal,thetasend,thetareceive,edge,tsend,treceive):
    if(edge):
...
t1=time.time()
numAgents = numUsers
sharing_delay =10000 # this equals buffer length
init_rounds=0
maxNumRounds = 50000
alpha = 0.03
alpha2 = 0.7
...
#p.close()  
    ...
    for k in range(0,numShares):
        ...
        total_ans=distData.map(lambda x: (Agentshare(x,listAgents[x],covDCCB[listAgents[x]],bDCCB[listAgents[x]],covBuffDCCB[listAgents[x]],
